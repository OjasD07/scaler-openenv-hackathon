...
`API_KEY` is preferred for the submission path because the grader injects the LiteLLM proxy credentials through that variable, but `OPENAI_API_KEY` is also accepted for local or Space setups.
